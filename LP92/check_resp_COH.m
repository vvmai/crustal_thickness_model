...
% set constants
    ...
    nx = 50;
    %stress data
    xs = logspace(log10(min(s_all))-1,log10(max(s_all))+3,nx);
    %temperature data
    ...
    normCOH = 3000; % Water content (ppm H/Si)
...
loglog(xs, 1.2e-8.*xs.^4.*exp(-152000/(normT*R)),'c', 'Linewidth', 1);
loglog(xs, 10^mean(log10(A_dis)).*xs.^mean(n).*normCOH^mean(r_dis).*exp(-mean(Q_dis)./(normT.*R)), 'k');
...
semilogy(1e4./xt, 1.2e-8*norms^4.*exp(-152000./(xt.*R)),'c', 'Linewidth', 1);
semilogy(1e4./xt, 10^mean(log10(A_dis))*norms^mean(n).*normCOH^mean(r_dis).*exp(-mean(Q_dis)./(xt.*R)), 'k');
...
loglog(xd, ones(size(xd)).*1.2e-8*norms^4.*exp(-152000./(normT.*R)),'c', 'Linewidth', 1);
loglog(xd, ones(size(xd)).*10^mean(log10(A_dis))*norms^mean(n).*normCOH^mean(r_dis)*exp(-mean(Q_dis)./(normT*R)), 'k');
%{
figure(4); hold off;
