...
template<class T>
inline
void Array1d<T>::sort_unique()
{
    vector<T> tmp(size());

    for (int i=1; i<=size(); i++) tmp[i-1] = (*this)(i);
    sort(tmp.begin(),tmp.end());

    typename vector<T>::iterator new_end = unique(tmp.begin(),tmp.end());
    tmp.erase(new_end,tmp.end());

    resize(tmp.size());
    for (int i=1; i<=size(); i++) (*this)(i) = tmp[i-1];
}
